...
export const getAllResources = async (client: Client) => {
  const resourceBundle = await client.request(
    `/Patient/${client.patient.id}/$everything`,
    {
      pageLimit: 0,
      flat: true,
      ...
      return bundle.entry.map((medication: Medication) => {
        return getMedicationName(
          client.getPath(
            ...
            client.getPath(
              medication,
              'resource.medicationReference.code.coding',
            ),
        );
      });
    }
...
export const formatName = (resource: Practitioner | Patient) => {
  const name =
    resource.name instanceof Array
      ? resource.name.find((e) => e.use === 'official') || resource.name[0]
      : resource.name;
  return name
